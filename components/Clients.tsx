...
const row1Clients = [
  ...
  },
  {
    name: 'Snowflake',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="w-8 h-8 text-cyan-400">
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 text-amber-500">
...
const row2Clients = [
  ...
  },
  {
    name: 'AWS Sagemaker',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="w-8 h-8 text-teal-400">
        ...
        <div className="flex w-full overflow-hidden">
          <div className="flex animate-marquee whitespace-nowrap gap-6 py-2">
            {marqueeRow1.map((logo, idx) => (
              <div
                key={`r1-${idx}`}
                className="w-48 h-40 bg-secondary/50 backdrop-blur-md border border-white/10 rounded-3xl p-6 flex flex-col items-center justify-between text-center flex-shrink-0 group hover:border-accent/40 transition-smooth shadow-glass cursor-pointer"
              >
